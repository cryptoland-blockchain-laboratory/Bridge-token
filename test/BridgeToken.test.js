...
contract("BridgeToken", (accounts) => {
  ...
  it("increaseAllowance", async () => {
    let balance1 = await token.allowance(accounts[0], accounts[1]);
    await token.increaseAllowance(accounts[1], 50 * TOKEN_UNIT);
    let balance2 = await token.allowance(accounts[0], accounts[1]);
    assert.equal(balance1, balance2 - 50 * TOKEN_UNIT);
  });

  // it("increaseAllowance", async () => {
  //   try {
  //     await token.approve(accounts[1], 0)
  //     await token.increaseAllowance(accounts[1], 20  * TOKEN_UNIT, { shouldPollResponse: true })
  //     assert(false, "didn't throw then ...")
  //   }catch(error) {
  //     console.log(error, 'REVERT opcode executed')
  //   }
  // })

  it("increaseAllowance event", async () => {
    return new Promise(async (resolve, reject) => {
      try {
        const _token = await tronWeb.contract().at(token.address);
        wait(3);
        const watcher = await _token.Approval().watch((err, res) => {
          ...
          if (res) {
            ...
            assert.equal(
              res.result.value,
              parseInt(allowance) + 50 * TOKEN_UNIT
            );
            console.log();
            watcher.stop();
            resolve();
  ...
  it("decreaseAllowance", async () => {
    let balance1 = await token.allowance(accounts[0], accounts[1]);
    await token.decreaseAllowance(accounts[1], 150 * TOKEN_UNIT);
    let balance2 = await token.allowance(accounts[0], accounts[1]);
    assert.equal(balance1, balance2 - 150 * TOKEN_UNIT);
  });

  // it('decreaseAllowance event', async () => {
  //   return new Promise(async (resolve, reject) => {
  //     try {
  //       const _token = await tronWeb.contract().at(token.address)
  //       wait(3)
  ...
  //           assert.equal(res.result.value, parseInt(allowance) - (30 * TOKEN_UNIT))
  //           console.log();
  //           watcher.stop()
  //           resolve()
  //         }
  //       })
  //       let allowance = await token.allowance(accounts[0], accounts[1])
  //       await token.decreaseAllowance(accounts[1], 30 * TOKEN_UNIT)
  //     } catch (e) {
  //       reject(e)
  //     }
  //   })
